...
def record_match(name, score, lvl):
    with connect_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT id FROM accounts WHERE nickname = %s
            """, (name,))
            acc_id = cur.fetchone()
            ...
            cur.execute("""
                INSERT INTO match_history (acc_id, points, difficulty_level)
                VALUES (%s, %s, %s)
            """, (acc_id[0], score, lvl))
    ...
    with connect_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                JOIN accounts a ON a.id = m.acc_id
...
def fetch_best_score(name):
    with connect_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT COALESCE(MAX(m.points), 0)
                FROM match_history m
                JOIN accounts a ON a.id = m.acc_id
                WHERE a.nickname = %s
            """, (name,))
