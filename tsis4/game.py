...
class SnakeEngine:
    ...
    def render(self):
        ...
        pygame.draw.circle(self.window, (255, 80, 80),
                           (self.target[0] + 10, self.target[1] + 10), 8)

        pygame.draw.rect(self.window, (100, 0, 0), (*self.trap, TILE, TILE))

        clr = self.cfg.get("snake_color", (0, 200, 0))

        for i, pos in enumerate(self.body):
            color = [max(0, c - 30) for c in clr] if i == 0 else clr
            pygame.draw.rect(self.window, color, (*pos, TILE - 1, TILE - 1), border_radius=5)
        ...
        while self.active:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_UP and self.vector != (0, TILE):
                        self.vector = (0, -TILE)
                    if event.key == pygame.K_DOWN and self.vector != (0, -TILE):
                        self.vector = (0, TILE)
                    if event.key == pygame.K_LEFT and self.vector != (TILE, 0):
                        self.vector = (-TILE, 0)
                    if event.key == pygame.K_RIGHT and self.vector != (-TILE, 0):
                        self.vector = (TILE, 0)
